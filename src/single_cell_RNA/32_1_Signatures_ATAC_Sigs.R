...
# Read genesets
genesets <- list()
f <- list.files(dirout("ATAC-seq_signatures"),pattern = "\\.csv")
fx <- f[1]
for(fx in f){
  genesets[[gsub("\\.csv", "", fx)]] <- fread(dirout("ATAC-seq_signatures/", fx), header=F)$V1
}
str(genesets)
names(genesets) <- gsub("gene_level", "", gsub("atac-seq", "",names(genesets)))
...
if(!file.exists(dirout(out,"Scores.RData"))){  
  ...
  fe.ret <- foreach(set.nam = names(genesets)) %dopar% {
  #fe.ret <- foreach(set.nam in names(genesets)){
    message(set.nam)
    genes <- genesets[[set.nam]]
    str(genes <- genes[genes %in% rownames(data)])
    ...
    return((score.x - bg.mean)/bg.sd)
  }
  names(fe.ret) <- names(genesets)
  fe.nam <- names(fe.ret)[1]
  for(fe.nam in names(fe.ret)){
    Dat1[[gsub("gene_level", "", gsub("atac-seq", "",fe.nam))]] <- fe.ret[[fe.nam]][Dat1$rn]
  }
...
ggplot(
  overTime.sig,#[grepl("top200.only_coding", geneset) & grepl("cluster_(0|1)", geneset)], 
  ...
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) + facet_grid(. ~ CellType,scales="free", space="free")
ggsave(dirout(out, "0_Overview1.pdf"),height=15, width=15)
