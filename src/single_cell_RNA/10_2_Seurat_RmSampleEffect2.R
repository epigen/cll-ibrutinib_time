...
sample.x <- "allDataBest_NoDownSampling_noIGH_RmSample_cellCycle"
...
if(!file.exists(dirout(outS, sample.x,".RData"))){
  ...
  ggplot(assignments$score, aes(x=G1, y=S, color=G2M)) + geom_point()
  ggsave(dirout(outS, "Cell_cycle_Plot.pdf"))
  
  for(n in colnames(assignments$score)){
          pbmc@data.info[,n] <- as.numeric(assignments$score[,n])
  }
  
  # regress out unwanted sources of variation (UMI normalization), also normalize for mitochondrial content
  pbmc <- RegressOut(pbmc, latent.vars = c("nUMI", "percent.mito", "sample", "G1", "G2M", "S")) #, model.use="negbinom"
