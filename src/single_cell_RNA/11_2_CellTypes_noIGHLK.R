...
if(!file.exists(dirout(outS, cell,".RData"))){
  ...
  cellToKeep.idx <- NA
  if(cell == "CD4"){
    cellToKeep.idx <- pbmcOrig@data.info[["ClusterNames_0.9"]] %in% c("9", "14")
  }
  if(cell == "CD8"){
    cellToKeep.idx <- pbmcOrig@data.info[["ClusterNames_0.9"]] %in% c("1", "2")
  }
  if(cell == "NKcells"){
    cellToKeep.idx <- pbmcOrig@data.info[["ClusterNames_0.9"]] %in% c("11")
  }
  if(cell == "Bcells"){
    cellToKeep.idx <- pbmcOrig@data.info[["ClusterNames_0.9"]] %in% c("0", "3", "4", "6", "7", "8", "10", "13", "17")
  }
  if(cell == "Monos"){
