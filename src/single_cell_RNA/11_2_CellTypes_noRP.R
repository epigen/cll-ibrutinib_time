...
if(!file.exists(dirout(outS, cell,".RData"))){
  ...
  cellToKeep.idx <- NA
  if(cell == "CD4"){
    cellToKeep.idx <- pbmcOrig@data.info[["ClusterNames_0.5"]] %in% "7"
  }
  if(cell == "CD8"){
    cellToKeep.idx <- pbmcOrig@data.info[["ClusterNames_0.95"]] %in% c("6", "2")
  }
  if(cell == "NKcells"){
    cellToKeep.idx <- pbmcOrig@data.info[["ClusterNames_0.5"]] %in% c("9")
  }
  if(cell == "Bcells"){
    cellToKeep.idx <- pbmcOrig@data.info[["ClusterNames_0.5"]] %in% c("0", "1", "3", "6", "8", "10")
  }
  if(cell == "Monos"){
    cellToKeep.idx <- pbmcOrig@data.info[["ClusterNames_0.5"]] %in% c("5")
  }
  if(cell == "NurseLikeCells"){
    cellToKeep.idx <- pbmcOrig@data.info[["ClusterNames_0.5"]] %in% c("11")
  }
  if(cell == "Tcells1"){
    cellToKeep.idx <- pbmcOrig@data.info[["ClusterNames_0.95"]] %in% c("14")
  }
