...
out <- "30_8_Signatures_RowNormalized_DownSampled/"
...
x <- pbmc@raw.data[rownames(pbmc@data), colnames(pbmc@data)]
...
quantile(x@j)
x2 <- x[,1,drop=F]
x3 <- data.table(i=x2@i, j=x2@j, x=x2@x)
x4 <- x3[sample(do.call(c, lapply(1:nrow(x3), function(row.i) rep(row.i, x3[row.i]$x))), min(nUMIs))]

nonzero.idx <- x@i[x@j == 0]
nonzero.val <- x@x[x@j == 0]
new.counts <- sample(do.call(c, lapply(1:length(nonzero.idx), function(idx) rep(nonzero.idx[idx], times=nonzero.val[idx]))), min(nUMIs))
length(new.counts) == min(nUMIs)
plot(ecdf(log10(nUMIs)))
