...
out <- "30_9_4_Signatures_nUMI_Cutoff_TFs/"
...
cllDownGenes <- fread(dirout("13_4_Overtime_nUMI_Cutoff/SigGenes_overTime.tsv"))
...
for(database in c("ENCODE_and_ChEA_Consensus_TFs_from_ChIP-X", "Transcription_Factor_PPIs")){
  fpath <- paste0("http://amp.pharm.mssm.edu/Enrichr/geneSetLibrary?mode=text&libraryName=",database)
  
  fhandle <- file(fpath)
  dblines <- readLines(con=fhandle)
  close(fhandle)
  ll <- dblines[which(grepl("^(NFKB|SPI|RUNX3|BATF|ATF2|NFIC|RELA)", dblines))]
  ll <- strsplit(ll, "\t")
  names(ll) <- sapply(ll, function(v) return(v[1]))
  ll <- lapply(ll, function(v) return(gsub("\\,1\\.0$", "", v[3:length(v)])))
  genesets <- c(genesets,ll)
}
lapply(genesets, tail)
