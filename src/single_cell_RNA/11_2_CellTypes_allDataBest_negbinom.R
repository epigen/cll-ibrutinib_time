...
if(!file.exists(dirout(outS, cell,".RData"))){
  ...
  cellToKeep.idx <- NA
  if(cell == "CD4"){
    cellToKeep.idx <- pbmcOrig@data.info[["ClusterNames_0.6"]] %in% "6"
  }
  if(cell == "CD8"){
    cellToKeep.idx <- pbmcOrig@data.info[["ClusterNames_0.6"]] %in% c("2", "4")
  }
  if(cell == "NKcells"){
    cellToKeep.idx <- pbmcOrig@data.info[["ClusterNames_0.6"]] %in% c("8") &
        pbmcOrig@data.info[["ClusterNames_0.95"]] %in% "12"
  }
  if(cell == "Bcells"){
    cellToKeep.idx <- pbmcOrig@data.info[["ClusterNames_0.6"]] %in% c("0", "1", "3", "9", "14")
  }
  if(cell == "Monos"){
    cellToKeep.idx <- pbmcOrig@data.info[["ClusterNames_0.6"]] %in% c("5", "10")
  }
  if(cell == "NurseLikeCells"){
    cellToKeep.idx <- pbmcOrig@data.info[["ClusterNames_0.6"]] %in% c("12") 
  }
  if(cell == "Tcells1"){
    cellToKeep.idx <- pbmcOrig@data.info[["ClusterNames_0.6"]] %in% c("8") &
        pbmcOrig@data.info[["ClusterNames_0.95"]] %in% "16"
  }
