...
sample.x <- "allDataBest_NoDownSampling_noIGH_RmSamplePCs"
...
if(!file.exists(dirout(outS, sample.x,".RData"))){
  ...
  pbmc <- RegressOut(pbmc, latent.vars = c("nUMI", "percent.mito")) #, model.use="negbinom"
  ...
  for(i in 1:length(pbmc@pca.rot)){
    message(i)
    print(anova(lm(pbmc@pca.rot[[i]] ~ pbmc@data.info$sample))$'Pr(>F)')
  }
